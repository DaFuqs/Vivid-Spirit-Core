...
import net.cosmicapiary.vivid_spirit.VividBlockTags;
...
public class PigmentSiphonBlockEntity extends LootableContainerBlockEntity implements PlayerOwnedWithName, BlockPosEventQueue.Callback<BlockPosEventQueue.EventEntry> {
    ...
    public PigmentSiphonBlockEntity(BlockPos blockPos, BlockState blockState) {
        super(VividBlockEntities.PIGMENT_SIPHON, blockPos, blockState);
    ...
    public static void tick(World world, BlockPos pos, BlockState state, PigmentSiphonBlockEntity blockEntity) {
        if (!world.isClient) {
     ...
     */
    private static void compensateGemstoneClusterDropsForUnloadedTicks(World world, BlockPos blockPos, PigmentSiphonBlockEntity blockEntity, long ticksToCompensate) {
    ...
    protected Text getContainerName() {
        if (hasOwner()) {
            return Text.translatable("block.vivid_spirit.pigment_siphon.owner", this.ownerName);
        } else {
            return Text.translatable("block.vivid_spirit.pigment_siphon");
        }
    }
    ...
    public boolean canAcceptEvent(World world, GameEventListener listener, GameEvent.Message message, Vec3d sourcePos) {
        return message.getEvent() == SpectrumGameEvents.BLOCK_CHANGED && !this.listenerPaused && message.getEmitter().affectedState().isIn(VividBlockTags.PIGMENT_SIPHON_HARVESTABLE);
    }
    ...
    public void triggerEvent(World world, GameEventListener listener, BlockPosEventQueue.EventEntry entry) {
        if (listener instanceof BlockPosEventQueue && this.getWorld() != null) {
            ...
            if (eventState.isIn(VividBlockTags.PIGMENT_SIPHON_HARVESTABLE)) {
                // harvest
